...
patch_file="$repo_root/patches/coreboot-starlabs_2607/0004-soc-intel-lockdown-allow-SPI-locking-in-SMM.patch"
...
grep -F '+	return fast_spi_ctrlr_protect_region(region, WRITE_PROTECT);' \
	"$patch_file" >/dev/null
grep -F '+			die("Unable to restore SPI boot-media protection before lockdown\n");' \
	"$patch_file" >/dev/null

protect_line=$(grep -n -F '+		if (fast_spi_flash_write_protect(&bootmedia) < 0)' \
	"$patch_file" | cut -d: -f1)
